...
def push_after_process_task(self, prev_task_results):
    project_id = prev_task_results.get('project_id')
    do_reset = prev_task_results.get('do_reset', False)
...
def process_and_push_workflow(self, project_id: int, file_id: str, chunk_size: int, overlap_size: int, do_reset: bool):
