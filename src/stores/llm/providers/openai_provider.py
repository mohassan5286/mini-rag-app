...
class OpenAIProvider(LLMInterface):
    def __init__(self, api_key: str, api_url: str = None, default_input_max_characters: int = 1000, default_output_max_tokens: int = 1000, default_generation_temperature: float = 0.1):
    ...
    def embed_text(self, text: str | list[str], document_type: str | None = None):
        if not self.client:
            self.logger.error("OpenAI client was not set")
            return None  
        ...
        response = self.client.embeddings.create(input=[ self.process_text(t) for t in text ], model=self.embedding_model_id)
